...
# **HOW TO USE:** 
Download the files and keep them in the same folder in the UNIX environment. Call "make game" and then "./game &lt; text file &gt; &lt; text file &gt;" to run the game. The first file is Alice's cards, and the second is Bob's. The text file must be formatted so that each line is: <suit> <number>, wchih represents a card. It will output all the matches that Alice and Bob make, then print out the remainder of their held cards. For testing purposes, call "make tests" and then "./tests" to run all the tests on functions of cards.cpp.
